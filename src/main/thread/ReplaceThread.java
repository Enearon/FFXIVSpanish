...
import main.replace.ReplaceEXDF;
import main.replace.ReplaceFont;
import main.swing.PercentPanel;
import main.swing.TextPatchPanel;
...
public class ReplaceThread implements Runnable {
	...
	public void run() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "<html><body>Traducción completa</body></html>", "Completado", -1);
			log.info("Patch finished.");
			percentPanel.dispose();
			this.textPatchPanel.replaceButton.setEnabled(true);
		} catch (Exception exception) {
			JOptionPane.showMessageDialog(null, "<html><body>Error del programa！</body></html>", "Error de traducción", 0);
